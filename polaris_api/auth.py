...
def create_api_token(client_name: str) -> str:
    ...
    return jwt_auth.create_token(
        user_id=f"api_client_{client_name}",
...
async def log_auth_attempt(request: Request, user_info: Optional[Dict] = None):
    ...
    if user_info:
        log_info(f"Authenticated request from {client_ip} - User: {user_info.get('user_id')}")
    else:
        log_warning(f"Unauthenticated request from {client_ip} - User-Agent: {user_agent}")
