...
SESSIONS = {name: {"session_id": name, "history": []} for name in USER_NAMES}
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                API_URL, json={"session_id": session_id, "prompt": message}, timeout=60
            )
...
async def generate_followup(session_name, resposta_anterior):
    ...
    followup = await send_message(
        session_name, prompt_shadow, session_id_override=shadow_id, show_logs=False
    )
